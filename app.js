...
const CATEGORIES = [
  {
    ...
    label: "餐厅",
    title: "附近餐厅",
    searchTexts: RESTAURANT_SEARCH_TEXTS,
    emptyTitle: "没搜到餐厅",
...
const BROAD_FOOD_QUERIES = ["", "餐饮", "美食"];
...
const demoResults = {
  restaurants: [
  ...
  },
    {
      ...
      name: "暖锅小馆",
      type: "火锅",
      address: "演示路 66 号 2 楼",
      distance: 246,
    rating: "4.8",
    cost: "88",
    location: "121.475667,31.232525",
    photo: "",
  },
    {
      id: "demo-3",
...
async function searchCategory(center, category) {
  ...
  for (const searchText of category.searchTexts) {
    const pageResults = await searchNearbyPages(center, searchText, {
      looseType: category.id === "dessert" && isDessertSearchText(searchText),
    });
    collected.push(...pageResults);

    const filtered = getSortedLimitedResults(filterByCategory(dedupeRestaurants(collected), category.id));
    if (filtered.length >= MAX_RESULTS_PER_CATEGORY) {
      return filtered;
    }
  }
...
async function searchNearbyPages(center, searchText, options = {}) {
  const results = [];

  for (let pageIndex = 1; pageIndex <= SEARCH_PAGES; pageIndex += 1) {
    const page = await searchNearbyPage(center, searchText, pageIndex, options);
    results.push(...page);
...
async function searchNearbyPage(center, searchText, pageIndex, options = {}) {
  const result = await requestAmap("around", {
    keywords: searchText,
    location: stringifyCenter(center),
    radius: SEARCH_RADIUS,
    types: options.looseType ? "" : "餐饮服务",
    offset: SEARCH_PAGE_SIZE,
    page: pageIndex,
    extensions: "all",
  });

  return (result?.pois || []).map((poi) => normalizePoi(poi, searchText));
}
...
async function searchDessertFallbackPool(center) {
  const collected = [];

  for (const searchText of BROAD_FOOD_QUERIES) {
    const pageResults = await searchNearbyPages(center, searchText);
    collected.push(...pageResults);
  }

  return getSortedLimitedResults(dedupeRestaurants(collected).filter(isDessertPlace));
}
...
function filterByCategory(list, categoryId) {
  ...
  if (categoryId === "dessert") {
    return foodList.filter(isDessertPlace);
  }
...
function isDessertSearchText(searchText) {
  return hasAnyKeyword(searchText.toLowerCase(), DESSERT_KEYWORDS);
}

function isDessertPlace(restaurant) {
  const text = getFilterText(restaurant);
  if (hasAnyKeyword(text, NON_FOOD_KEYWORDS)) {
    return false;
  }
...
function isRestaurantPlace(restaurant) {
  const text = getFilterText(restaurant);
  if (hasAnyKeyword(text, RESTAURANT_EXCLUDE_KEYWORDS)) {
    return false;
  }
...
function isFoodRelatedPlace(restaurant) {
  ...
  return (
    hasAnyKeyword(text, [...RESTAURANT_INCLUDE_KEYWORDS, ...DESSERT_KEYWORDS]) ||
    hasAnyKeyword(restaurant.sourceQuery || "", [...DESSERT_KEYWORDS, ...BROAD_FOOD_QUERIES])
  );
}
...
function normalizePoi(poi, sourceQuery = "") {
  const location =
    typeof poi.location === "string"
      ? poi.location
      : poi.location
        ? `${poi.location.lng ?? poi.location.getLng?.()},${poi.location.lat ?? poi.location.getLat?.()}`
        : "";
...
function formatAddress(address) {
  if (Array.isArray(address)) {
    return address.filter(Boolean).join(" ") || "暂无地址";
  }
  return address || "暂无地址";
}
...
function renderActiveCategory() {
  ...
  els.resultCount.textContent = list.length
    ? `${category.label} · 找到 ${list.length} 家`
    : "没有结果";
  ...
  list.forEach((restaurant) => {
    ...
    if (restaurant.photo) {
      photo.style.backgroundImage = `url("${restaurant.photo}")`;
    }
...
function buildMeta(restaurant) {
  ...
  if (restaurant.distance) {
    parts.push(`${Math.round(restaurant.distance)} 米`);
  }
  if (restaurant.rating) {
    parts.push(`评分 ${restaurant.rating}`);
  }
  if (restaurant.cost) {
    parts.push(`人均 ¥${restaurant.cost}`);
  }
...
function playRandomSpin() {
  ...
  window.setTimeout(() => {
    els.randomButton.classList.remove("is-spinning");
  }, 700);
}
...
function buildNavigationUrl(restaurant) {
  ...
  return `https://uri.amap.com/marker?position=${longitude},${latitude}&name=${encodeURIComponent(
    restaurant.name,
  )}`;
}
...
function updateSuggestions() {
  ...
  if (!keyword || keyword.length < 2 || !getHasAmapProxy()) {
    hideSuggestions();
    return;
  }
...
async function locateCurrentPosition({ force = false } = {}) {
  ...
  try {
    ...
    setState(
      ...
      isPermissionDenied
        ? "请在浏览器地址栏附近重新允许定位权限，或者直接输入地址并从下拉框选择位置。"
        : `${message}。你也可以手动输入地址，再从下拉框里选择最匹配的位置。`,
      "error",
    );
  } finally {
...
async function runSearch({ address, center = null }) {
  ...
  try {
    if (!getHasAmapProxy()) {
      resultsByCategory = {
        restaurants: demoResults.restaurants,
        dessert: demoResults.dessert,
      };
      isShowingDemo = true;
    ...
    const restaurantCategory = CATEGORIES.find((category) => category.id === "restaurants");
    const dessertCategory = CATEGORIES.find((category) => category.id === "dessert");
    const restaurants = await searchCategory(resolvedCenter, restaurantCategory);
    const desserts = await searchCategory(resolvedCenter, dessertCategory);
    const broadDessertFallback =
      desserts.length < 5 ? await searchDessertFallbackPool(resolvedCenter) : [];

    resultsByCategory = {
      restaurants,
      dessert: desserts,
    };
    resultsByCategory.dessert = dedupeRestaurants([
      ...(resultsByCategory.dessert || []),
      ...broadDessertFallback,
      ...getDessertFallback(resultsByCategory.restaurants || []),
    ]);
    resultsByCategory.dessert = getSortedLimitedResults(resultsByCategory.dessert);
    isShowingDemo = false;
    renderActiveCategory();
...
els.closePickedButton.addEventListener("click", () => {
  els.pickedPanel.classList.add("hidden");
});
els.pickedPanel.addEventListener("click", (event) => {
  if (event.target === els.pickedPanel) {
    els.pickedPanel.classList.add("hidden");
  }
});
els.tabButtons.forEach((button) => {
...
els.addressInput.addEventListener("click", () => {
  updateSuggestions();
});
